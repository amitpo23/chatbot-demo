...
async function main() {
  ...
  // Read the B2B industry knowledge base
  const text = readFileSync("public/knowaa-b2b-industry-knowledge.txt", "utf-8");
  console.log(`B2B Knowledge base: ${text.length} characters`);

  // Split by sections (---) first, then by sub-sections (###)
  ...
  for (const section of majorSections) {
    ...
    // Split by sub-sections for better semantic coherence
    const subSections = trimmed.split(/\n### /);

    for (let j = 0; j < subSections.length; j++) {
      let sub = subSections[j].trim();
      if (sub.length === 0) continue;

      // Re-add ### prefix (except for the first chunk which has the ## header)
      if (j > 0) sub = "### " + sub;

      if (sub.length <= CHUNK_SIZE) {
        allChunks.push(sub);
      } else {
        const chunks = chunkText(sub, CHUNK_SIZE, CHUNK_OVERLAP);
        allChunks.push(...chunks);
      }
    }
  }

  console.log(`Created ${allChunks.length} chunks from B2B knowledge`);
  ...
  console.log("Done! B2B knowledge base ingested successfully.");
